...
const App = {
  ...
  // integrateWallet: function (account) {
  //   const walletInstance = cav.klay.accounts.privateKeyToAccount(privateKey);
  //   cav.klay.accounts.wallet.add(walletInstance);
  ...
  changeUI: async function (walletInstance) {
    ...
    cav.klay.getBalance(walletInstance.address)
          .then(function (balance) {
              document.getElementById("myWallet").innerHTML = `지갑주소: ${walletInstance.address}`
  ...
  receiveKlay: async function () {
    ...
    .then(function (receipt) {
      if (receipt.status){
        spinner.stop();
        alert("0.1 KLAY가 " + account + " 계정으로 지급되었습니다.");
        $('#transaction').html("");
